...
class ECGHome : AppCompatActivity() {
    ...
    var ecgDataQueue = LinkedList<Int>()
    var timeStampList = mutableListOf<Long>()
    var timeStampQueue = LinkedList<Long>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnReconnect.setOnClickListener {
            lifecycleScope.launch(Dispatchers.IO) {
                ...
                var prevLoopTime = System.currentTimeMillis()
                val startTime = prevLoopTime
                while (true) {
                    try {
                        ...
                        withContext(Dispatchers.Main) {
                            ...
                            ecgDataQueue.push((compositeData % 10000).toInt())
                            //timeStampList.add(compositeData / 10000)
                            timeStampQueue.push(compositeData / 10000)
                        }
                    ...
                    if ((System.currentTimeMillis() - startTime) > 20000 && System.currentTimeMillis() - prevLoopTime > 2000) {
                        val firstTimeStamp = timeStampQueue[0]
                        while (timeStampQueue[0]-firstTimeStamp<2000) {
                            ecgDataQueue.pop()
                            timeStampQueue.pop()
                        }
                        ...
                        try {
                            ...
                            withContext(Dispatchers.Main) {
                                val dataList = module.callAttr(
                                    "get_bpm_metric",
                                    ecgDataQueue.toIntArray(),
                                    timeStampQueue.toLongArray()
                                ).asList()
                                binding.tvHeartRate.text = dataList[0].toString()
